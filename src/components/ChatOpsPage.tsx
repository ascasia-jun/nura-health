                    ...
                    {(sessions || []).map(s => (
                        <div key={s.id} className="relative group/session">
                            {editingSessionId === s.id ? (
                                ...
                                <button onClick={async () => { const drafted = await loadSession(s, input); setInput(drafted); }} className={`w-full flex items-center justify-between gap-3 px-4 py-2 rounded-lg transition-colors text-sm text-left truncate ${currentSessionId === s.id ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'text-slate-400 hover:bg-white/5'}`}>
